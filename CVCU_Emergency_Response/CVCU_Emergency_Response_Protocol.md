...
| **アドレナリン 1mg** | 3–5分毎に投与 (Class 2a-B-R [3]) | **ルーチン投与禁止**（Class III-Harm [2]）— senior医師指示下のみ |
...
- 目標 systolic ≥ **60 mmHg**（外マッサージと同じ目標。STS 2017／EACTS 2009 とも内心臓マッサージで「>60 mmHg」を目安）[2]
...
2. **血行動態評価**
   ...
   - **安定** → レート調整 or リズム調整（どちらも妥当, Class IIb-B-R [6]）→ §7.3 / §7.4
...
- 直接経口抗凝固薬(DOAC) > ビタミンK拮抗薬(ワーファリン)（出血少、Class IIb-B-NR [6]）
...
| 策 | 推奨度 | 内容 |
|---|---|---|
| **経口アミオダロン** | **Class I-B-R** | 周術期予防的経口投与 |
| β-blocker 継続 | Class IIa-B-R | 既存β-blocker 患者は術前朝に最終投与、POD1 から再開 |
| 左後心膜切開 | Class IIa-B-R | 術中操作 |
| 経静脈マグネシウム | Class IIb-B-R | 周術期 |
| コルヒチン | Class IIb-B-R | COPPS protocol |
| 両心房ペーシング | Class IIb-B-R | 術中設置時 |
| **ルーチン K 補正** | **Class III (no benefit), B-R** | TIGHT-K trial で否定 |
...
| 静脈側 | **21–23 Fr** | 大腿静脈 |
| 遠位灌流カテーテル | **4–7 Fr** | 同側浅大腿動脈 — 下肢虚血予防（推奨, クラス付与なし [7]） |
...
| 脳出血 | 1.8–21.3% | 瞳孔・GCS（毎時） | 頭部CT、抗凝固見直し |
...
- ERAS Cardiac 推奨: 術後6時間以内の抜管 (Class IIa-B-NR [8])
- Low tidal volume戦略
- 早期抜管達成のため鎮静を慎重に減量
- CVCU 移行時はすでに抜管後が多い
...
- **ICDSC** を **看護シフトごと**に実施 (Class I-B-NR [8])
...
- **stripping は禁止** (Class III-A [8] — No Benefit／無効。Table 1。※「III-Harm」は CPB復温時の高体温>37.9℃ に割当)
...
> 本プロトコルの推奨クラス・エビデンスレベル・数値を、引用8ガイドラインの原典PDF（EACTS 2009 CALS／STS 2017 CSU-ALS／AHA 2025 ALS／JRC 2023 ACS／ESC 2022 VA-SCD／STS 2026 POAF／JCS 2023 PCPS-ECMO-IMPELLA／ERAS Cardiac 2019）と逐語照合し、以下を修正のうえ確認済み。
>
> - **AHA 2025（最重要原則）：アドレナリン 1mg 3–5分毎**：Class 1-B → **Class 2a-B-R**（「1mgを3–5分毎」に一致する推奨は COR 2a・LOE B-R。「エピネフリン投与は推奨」自体は COR 1・B-R）
> - **STS 2017（§2.4）：心外膜ペーシングワイヤー接続**：Class IIa-C → **Class I-C**
> - **STS 2017／EACTS 2009（§3.5）：内心臓マッサージ目標圧**：≥80 mmHg → **≥60 mmHg**（両原典とも "systolic impulse more than 60 mm Hg"）
> - **STS 2026（§7.2）：安定POAF レート vs リズム**：Class IIb-B-NR → **Class IIb-B-R**（Statement 13）
> - **STS 2026（§7.5）：DOAC＞VKA**：Class IIb-B → **Class IIb-B-NR**（Statement 15）
> - **STS 2026（§7.6 予防）：エビデンスレベル明確化**：経口アミオダロン I-B→**I-B-R**、β遮断薬/左後心膜切開 IIa-B→**IIa-B-R**、Mg/コルヒチン/両心房ペーシング IIb-B→**IIb-B-R**、K補正 III(no benefit)→**III(no benefit), B-R**（全StatementのLOEはB-R）
> - **JCS 2023（§10.1）：遠位灌流カテーテル**：Class I-B → **クラス付与なし（推奨）**（本Focused Updateは「クラス・エビデンスレベルを付与せず＝専門家コンセンサス」と明記）
> - **JCS 2023（§10.6）：脳出血の頻度**：30–60% → **1.8–21.3%**（30–60% は出血合併症全体の頻度。脳出血は1.8–21.3%）
> - **ERAS Cardiac 2019（§11.2）：6時間以内抜管**：Class B-NR → **Class IIa-B-NR**（COR欠落を補完）
> - **ERAS Cardiac 2019（§11.4）：せん妄スクリーニング(ICDSC)**：Class C-LD → **Class I-B-NR**
> - **ERAS Cardiac 2019（§11.7）：胸腔ドレーン stripping 禁止**：Class III-Harm → **Class III「No Benefit」, Level A**（Table 1。III-Harm は CPB復温時高体温＞37.9℃ に割当）
>
> 上記以外の推奨クラス・レベル・閾値（3連続ショック I-B、エピネフリン投与禁止 III-Harm、アミオダロン300mg IIa-A、アトロピン III-no benefit、5分以内再開胸 I-C、抗菌薬追加 IIa-B〔STS 2017〕、電気的ストーム/TdP の各推奨〔ESC 2022〕、POAF疫学・カルジオバージョン I/C-EO〔STS 2026〕、SSIバンドル・AKI/KDIGO〔ERAS〕、IMPELLA仕様・カニューレ径〔JCS 2023〕等）は原典と一致を確認。
>
> **検証範囲の制限**：本プロトコルの薬剤投与量の多くは標準ACLS/添付文書/慣用量であり、引用ガイドラインが個別に数値を明示していないものがある（例：JRC 2023 は薬剤用量を記載せず、ECG「10分以内」や NSTE-ACS「＜24h/高リスク＜2h」も非収載＝ESC/ACC由来；ESC 2022 はMg・エスモロール用量や除細動エネルギー値を非収載＝ランジオロールは一致；JCS 2023 はヘパリン50U/kg・抗Xa・MAP/ScvO₂数値や「ECMO中CPRは継続せず」の文言を非収載；ERAS は§1/§11.1 の数値トリガーを非収載＝カテゴリのみ）。これらは値が臨床的に妥当なため改変せず、出典範囲のみ本注記で明示。
